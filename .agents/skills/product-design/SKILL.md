...
2. `../../../references/product-design-handoff.md`의 디자인 인계 점검(`DHC-*`)을 `design/00-design-context.md`의 `인계 점검`에 기록한다. 기획에서 판정해 넘긴 결과가 있으면 그대로 쓰고, 없으면 직접 판정한다. `먼저 묻는다` 항목이 비어 있으면 Phase 1의 질문에 포함한다.
3. 다음 중 실행 유형을 정한다.
   - 디자인 기반 없음: 초기 구축
   - 기존 기반으로 새 기능·화면 설계: UI 실행
   - 특정 산출물 수정: 부분 재실행
   - 다른 서비스·브랜드: 기존 결과를 덮어쓰지 않고 출력 범위를 사용자와 구분
4. 기존 확정 결정과 사용 중인 토큰·컴포넌트는 사용자 요청 없이 변경하지 않는다.
5. 중간 결과와 검토 근거가 필요하면 `design/_workspace/`에 저장하고 삭제하지 않는다.
